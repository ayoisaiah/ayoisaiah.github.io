...
<figure>
	<img src="{{ site.baseurl }}/assets/images/vuejs-to-do-app.png" alt="Vue.js Beginners Tutorial">
</figure>
...
Now that we've added Vue to our page, it's time to setup our HTML. We will create a main container `div` with an id of `events` and a new Vue instance and then bind the two together.
...
By setting the `el` key to `#todo`, this makes all the elements inside `div#todo` available to the Vue instance.
...
Since this article is not about CSS, I'm not going to dwell too much on this section. Feel free to copy and paste this part of the tutorial.

## Getting to the nitty-gritty

We can start demonstrating the awesome capabilities of Vue.js by giving our to-do app some functionality.

But first, lets go over the concepts we need to understand about Vue.js so that you can follow along.
...
By setting the `el` key to `#todo`, this makes all the elements inside the `div#todo` available to the Vue instance. The `data` object is where we will register the values that hold data for our application while `methods` will hold all the methods we want to use in our application.
...
Continuing from the previous example, lets add a button that will call a function that alerts the name in the input field when the button is clicked.

<iframe width="100%" height="300" src="//jsfiddle.net/ayoisaiah/pdj3nc6j/6/embedded/result,html,js/" allowfullscreen="allowfullscreen" frameborder="0"></iframe>
...
Hopefully, all that makes sense. It's time to combine the concepts we've learnt above to add some functionality to our to-do list app.

### Feature 1 - Add a to-do item and display it on the page

The first feature we will implement in our app is adding a single to-do item to the list through the input field in the top panel.
...
If `task` evaluates to true, which it would except if `task` is an empty string, push an object to the `taskList` array with two properties: `text` is set to the task variable  while `checked` is set to false. `checked` is what we will use to track the checked state of each task.
...
Remember `taskList` is where our to-dos are stored so all we need to do to clear all items in our list is to set `taskList` to an empty array.

<iframe width="100%" height="300" src="//jsfiddle.net/ayoisaiah/v4p20ekz/20/embedded/result,js,html,css" allowfullscreen="allowfullscreen" frameborder="0"></iframe>
...
Remember we already set the `checked` property on each task to `false`  when we add a new to-do item. We only need a way to toggle it to true when a task is checked and vice versa.
...
<iframe width="100%" height="300" src="//jsfiddle.net/ayoisaiah/v4p20ekz/19/embedded/result,js,html,css" allowfullscreen="allowfullscreen" frameborder="0"></iframe>

---

That concludes my tutorial. I hope you have learnt the basics of Vue.js and how you can use it to build web interfaces. Would love to hear some feedback so please say what you think.
